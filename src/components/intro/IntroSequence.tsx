...
export function IntroSequence({ stage, onSkip, onDelhiClick }: IntroSequenceProps) {
  ...
  const [delhiPulse, setDelhiPulse] = useState(false);
  ...
  useEffect(() => {
    ...
    const sensorFull = "LOADING ATMOSPHERIC DATA.....";
    let i = 0, j = 0;
    ...
    const loadInterval = setInterval(() => {
      ...
      setLoadingProgress(Math.min(elapsed / 3000, 1));
      if (elapsed >= 3000) clearInterval(loadInterval);
    }, 16);
    return () => { clearInterval(typingInterval); clearInterval(sensorInterval); clearInterval(loadInterval); };
  }, [stage]);

  // Stage 2: "FINDING COUNTRY: INDIA" typed text
  useEffect(() => {
    if (stage !== 2) return;
    setFindingText("");
    const full = "FINDING COUNTRY: INDIA — 20.5°N 78.9°E";
    let i = 0;
    const t = setInterval(() => {
      if (i <= full.length) { setFindingText(full.slice(0, i)); i++; } else clearInterval(t);
    }, 55);
    return () => clearInterval(t);
  }, [stage]);

  // Stage 3: Show "click Delhi" hint after delay
  useEffect(() => {
    if (stage !== 3) return;
    setDelhiPulse(true);
    const t = setTimeout(() => setShowClickHint(true), 1500);
    return () => clearTimeout(t);
  }, [stage]);
  ...
  useEffect(() => {
    ...
    for (const url of tryTextures) {
      ...
      textureLoader.load(url, (texture) => {
        ...
        earth.material = new THREE.MeshPhongMaterial({ map: texture, specular: new THREE.Color(0x333333), shininess: 15 });
      }, undefined, () => {});
    }
    ...
    const atmMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `varying vec3 vNormal; void main() { float intensity = pow(0.55 - dot(vNormal, vec3(0,0,1)), 3.0); gl_FragColor = vec4(0.15, 0.55, 1.0, 1.0) * intensity; }`,
    ...
    const glowMat = new THREE.ShaderMaterial({
      vertexShader: `varying vec3 vNormal; void main() { vNormal = normalize(normalMatrix * normal); gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
      fragmentShader: `varying vec3 vNormal; void main() { float intensity = pow(0.65 - dot(vNormal, vec3(0,0,1)), 2.0); gl_FragColor = vec4(0.15, 0.5, 1.0, 1.0) * intensity; }`,
    ...
    function createMarker(lat: number, lon: number, color: number, size: number): THREE.Group {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const ringMat = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.5 - i * 0.15, side: THREE.DoubleSide });
    ...
    const delhiMarker = createMarker(DELHI_LAT, DELHI_LON, 0xff3333, 0.06);
  ...
  // Stage 2: Earth rises + rotate + show "Finding Country India"
  useEffect(() => {
    ...
    // Rotate to show India region
    const indiaTargetY = lonToEarthRotationY(INDIA_LON);
    gsap.to(earth.rotation, { y: indiaTargetY, duration: 3.5, ease: "power2.inOut" });
  }, [stage]);

  // Stage 3: Zoom into India, show Delhi marker, wait for click
  useEffect(() => {
    if (stage !== 3 || !sceneRef.current) return;
    const { camera, earth, stars, delhiMarker } = sceneRef.current;
  ...
  const handleDelhiClick = () => {
    if (stage !== 3 || !sceneRef.current) return;
    const { camera, stars } = sceneRef.current;

    // Zoom in dramatically then fade out
    gsap.to(camera.position, { z: 1.8, y: 0.3, duration: 1.5, ease: "power3.inOut" });
    gsap.to(stars.material as THREE.PointsMaterial, { opacity: 0, duration: 1 });

    if (containerRef.current) {
      gsap.to(containerRef.current, {
        opacity: 0, scale: 0.5, duration: 1.5, ease: "power2.in", delay: 0.5,
        onComplete: () => onDelhiClick?.(),
      });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-hidden bg-black">
        ...
        {stage === 2 && (
          ...
          >
            <p className="font-mono text-sm tracking-[0.2em] text-primary text-glow-primary">
              🌏 {findingText}<span className="animate-pulse-live">_</span>
            </p>
      ...
      {/* Stage 3: India zoomed — Delhi pinpointed, click to enter */}
      <AnimatePresence>
        {stage === 3 && (
          <motion.div
          ...
          >
            ...
            {showClickHint && (
              <motion.button
                ...
                onClick={handleDelhiClick}
                className="font-mono text-xs tracking-[0.2em] text-primary border border-primary/40 px-6 py-2.5 rounded-full hover:bg-primary/10 hover:border-primary/70 transition-all cursor-pointer backdrop-blur-sm"
                style={{ textShadow: "0 0 8px hsl(var(--primary) / 0.5)" }}
              >
                <span className="animate-pulse-live">▶</span> ENTER DELHI DASHBOARD
              </motion.button>
            )}
          </motion.div>
        )}
