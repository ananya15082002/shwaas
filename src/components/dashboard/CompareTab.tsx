...
export function CompareTab({ stations }: CompareTabProps) {
  ...
  useEffect(() => {
    if (selectedStations.length >= 2) {
      analyze(selectedStations, "compare", undefined, undefined, lang);
    }
  }, [selected.join(","), lang]);
              ...
              <div className="mt-2 h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={groupedData} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                    <XAxis dataKey="pollutant" tick={{ fill: "hsl(215, 12%, 50%)", fontSize: 10, fontFamily: "JetBrains Mono" }} axisLine={{ stroke: "hsl(220, 15%, 18%)" }} tickLine={false} />
